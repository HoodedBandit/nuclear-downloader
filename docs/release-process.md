...
On elevated runners, the launcher temporarily grants only the current user's SID access to the runner's desktop and window station, since their existing permissions may rely on the Administrators group that the worker intentionally lacks. The original permissions are restored after job cleanup, and fixtures verify exact restoration on success and timeout. No other user is granted access.
