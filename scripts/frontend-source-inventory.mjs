...
const OWNERSHIP = new Map([
  ...
  [
    ...
    {
      responsibility: 'Declare the renderer-only static application layout mode.',
      workflows: ['startup']
    }
  ],
  [
  ...
  [
    ...
    {
      responsibility: 'Own renderer snapshot/delta application and resynchronization sequencing.',
  ...
  [
    ...
    {
      responsibility: 'Derive stable operation and published-output facts from backend contracts.',
  ...
  [
    ...
    {
      responsibility: 'Provide the typed command and event boundary used by renderer workflows.',
  ...
  [
    ...
    {
      responsibility: 'Order and reduce operation progress without regressing terminal state.',
  ...
  [
    ...
    {
      responsibility: 'Own bounded renderer waiters for operation completion and teardown.',
...
export function inventoryTypeScript({
  ...
  const kind = scriptKind ?? (relativePath.endsWith('.js') ? ts.ScriptKind.JS : ts.ScriptKind.TS);
  ...
  function visit(node, lexicalOwner) {
    ...
    if (isFunctionLike(node)) {
      ordinal += 1;
      const identity = callableIdentity(node, sourceFile, lexicalOwner, ordinal);
      ...
      childOwner = lexicalOwner ? `${lexicalOwner}.${node.name.text}` : node.name.text;
    }
...
function templateEntry({ node, relativePath, source, owner, symbol, classification }) {
  ...
  function visit(node, owner, ancestors) {
    ...
    if (node.type === 'SnippetBlock') {
      ...
      entries.push(
        templateEntry({
          ...
          owner,
          symbol,
          classification: node.async ? 'markup-async-callback' : 'markup-callback'
        })
      );
      childOwner = `${owner}.${symbol}`;
    }
    for (const [key, value] of Object.entries(node)) {
      ...
      if (Array.isArray(value)) {
        for (const child of value) visit(child, childOwner, [...ancestors, node]);
      } else {
...
export async function buildInventory(frontendRoot = FRONTEND_ROOT) {
  ...
  for (const item of paths) {
    ...
    if (!ownership)
      throw new Error(`Production source lacks an ownership mapping: ${item.relative}`);
    const fileEntries = item.relative.endsWith('.svelte')
      ...
      : inventoryTypeScript({
          ...
          fullSource: source,
          scriptKind: item.relative.endsWith('.js') ? ts.ScriptKind.JS : ts.ScriptKind.TS
        });
    files.push({
      path: item.relative,
      sourceHash: sha256(source),
  ...
  for (const entry of entries) {
    if (ids.has(entry.id)) throw new Error(`Duplicate inventory identity: ${entry.id}`);
    ids.add(entry.id);
  }
  return {
    schemaVersion: 1,
    ...
    summary: {
      ...
      asyncCallbacks: entries.filter((entry) => entry.classification.includes('async-callback'))
        .length
    },
    files,
    entries
...
export async function checkInventory(output, frontendRoot = FRONTEND_ROOT) {
  ...
  try {
    ...
    throw new Error(`Frontend inventory is missing or unreadable: ${error.message}`);
  }
  if (actual !== expected) {
    throw new Error(
...
function parseArguments(argv) {
  ...
  let output = path.join(REPOSITORY_ROOT, 'docs', 'frontend-source-inventory.json');
  let check = false;
