...
try {
    ...
    Assert-True -Condition ($publishWorkflow.Contains('PUBLISH v0.7.9')) -Message 'Publish workflow must require the exact maintainer confirmation.'
    ...
    $manifestName = 'nuclear-downloader-v0.7.9-update.json'
    $legacyName = 'nuclear-downloader-v0.7.9-sha256.txt'
    ...
    Assert-True -Condition ($verificationOutput.Count -eq 1 -and $verificationOutput[0] -match '^Verified Nuclear Downloader 0\.7\.9 candidate') -Message 'Valid fixture candidate did not pass verification.'
